...
def bb_func(df, cal_col='',N1=20, N2=2):
    df['up'], df['me'], df['dn'] = talib.BBANDS(df[cal_col], timeperiod=N1, nbdevup=N2, nbdevdn=N2, matype=0)
    df['up'], df['me'], df['dn'] = df['up'].shift(), df['me'].shift(), df['dn'] .shift()




    open_con0 = df[cal_col] > df['me']
    open_con0 &= df[cal_col].shift() < df['me'].shift()

    open_con = df[cal_col] > df['up']
    open_con &= df[cal_col].shift() < df['up'].shift()
    open_con &= df['me'].diff(20)>0

    close_open_con = df[cal_col] < df['dn']
    close_open_con &= df[cal_col].shift() > df['dn'].shift()


    df.loc[open_con0,'pos'] = 1
    df.loc[open_con,'pos'] = 1
    df.loc[close_open_con,'pos'] = 0

    # df['up_close'] = 100 * (df[cal_col] / df['up'] - 1)
    # df.loc[df['up_close'] < 0, 'up_close'] = df['up_close'].abs()
    # # df['up_up'] = df['up_close'].rolling(N1,min_periods=2).max().shift()
    # df['up_up'], _, _ = talib.BBANDS(df['up_close'], timeperiod=N1, nbdevup=4, nbdevdn=4, matype=0)
    # df.loc[(df['up_close'].shift()<df['up_up'].shift()) &(df['up_close']>df['up_up']),'pos'] = 0
    df['pos'] = df['pos'].ffill().fillna(0)


    return df

def atr_func(df, cal_col='',N1=20, N2=2):
    df['me'] = talib.SMA(df[cal_col], timeperiod=N1)
    df['atr'] = talib.ATR(df[cal_col].rolling(3).max(), df[cal_col].rolling(3).min(0), df[cal_col], timeperiod=N1)

    df['up'] =  df['me'] + N2 *df['atr']
    df['dn'] =  df['me'] - N2 *df['atr']

    df['up'], df['me'], df['dn'] = df['up'].shift(), df['me'].shift(), df['dn'] .shift()


    # open_con = df[cal_col] > df['me']
    # open_con &= df[cal_col].shift() < df['me'].shift()

    open_con1 = df[cal_col] > df['up']
    open_con1 &= df[cal_col].shift() < df['up'].shift()
    open_con1 &= df[cal_col].diff(100)
    # open_con
    close_open_con = df[cal_col] < df['me']
    close_open_con &= df[cal_col].shift() > df['me'].shift()
    close_open_con &= df['jz'].diff(100)<0

    close_open_con1 = df[cal_col] < df['dn']
    close_open_con1 &= df[cal_col].shift() > df['dn'].shift()
    close_open_con1 &= df['jz'].diff(100)>0


    # df.loc[open_con,'pos'] = 1
    df.loc[open_con1,'pos'] = 1
    df.loc[close_open_con,'pos'] = 0
    df.loc[close_open_con1,'pos'] = 0
...
def mm_func(df, cal_col='',N1=20, N2=2):
    df['dn'] ,df['up'] = talib.MINMAX(df[cal_col], timeperiod=N1)
    df['up'],df['dn'] = df['up'].shift(), df['dn'] .shift()
    df['me'] = (df['up'] + df['dn']) / 2
    # df['dn'] = df['up'].rolling(10).min()
    # df['dn'] = df['dn'].shift(N2)

    open_con = df[cal_col] > df['up']
    open_con &= df[cal_col].shift() < df['up'].shift()
    # open_con &= df['me'].diff(N2)>=0
    df.loc[open_con,'pos'] = 1

    close_open_con = df[cal_col] < df['dn']
    close_open_con &= df[cal_col].shift() > df['dn'].shift()
    # close_open_con &= df['me'].diff(N2)>=0
    df.loc[close_open_con,'pos'] = 0
    #
    # close_open_con1 = df[cal_col] < df['me']
    # close_open_con1 &= df[cal_col].shift() > df['me'].shift()
    # close_open_con1 &= df['me'].diff(N2)<=0
    # df.loc[close_open_con1,'pos'] = 0
...
def ma_func(df, cal_col='',N1=20, N2=2):
    df['xma']  = talib.WMA(df[cal_col], timeperiod=N2)
    df['ma']  = talib.SMA(df[cal_col], timeperiod=N1)
    df['dn'] = talib.MIN(df[cal_col], timeperiod=N2)


    open_con = df['xma'] > df['ma']
    open_con &= df['xma'].shift() < df['ma'].shift()
    open_con &= df['xma'].diff(N2)>=0

    open_con1 =( df[cal_col] < df['xma']) &(df[cal_col].shift() >= df['xma'].shift())|( df[cal_col] > df['ma']) &(df[cal_col].shift() <= df['ma'].shift())
    open_con1 &= df['xma'] > df['ma']
    open_con1 &= df['xma'].diff(N2)>=0

    close_open_con = df['xma'] < df['ma']
    close_open_con &= df['xma'].shift() > df['ma'].shift()
    close_open_con &= df['xma'].diff(10)< 0
...
if __name__ == "__main__":
    if 0:
        ...
        df = pd.read_csv(rf"D:\jason_src\策略多因子系统\15min_全品种优化\backtest_result_data-f-2_s-4_e-2_jzmode-d_new\optimization_CLmain\选中策略情况\CLmain_实盘参数.csv")
        ...
        market_jz[combo_name].plot()
        plt.plot(market_jz['close']/market_jz['close'].iloc[0],label='close')

        plt.xticks( market_jz.index[::50],market_jz['datetime'].iloc[::50],rotation=45)
        plt.legend('left')
        plt.show()
    ...
    if 0*1:
        ...
        code_id = "CLmain"
        pos_path = fr"D:\jason_src\策略多因子系统\15min_全品种优化\backtest_result_data-f-2_s-4_e-2_jzmode-d_new\optimization_{code_id}\选中策略情况\{code_id}_all_position.csv"
        dfpos = pd.read_csv(pos_path)
        dfpos['datetime'] = pd.to_datetime(dfpos['datetime'],utc=True)
        cl_col = [col for col in dfpos.columns if "|" in col]
        mk_col = [col for col in dfpos.columns if "|" not in col]
        for clname in cl_col:
            dfpos[clname] = dfpos[clname]
            dfpos[clname] = dfpos[clname].apply(lambda x: 1 if x>0 else -1)
            cum_curve, all_metrics = compute_backtest_metrics_with_jz(
                                            market_df=dfpos[mk_col+[clname]],
                                            position_series= dfpos[clname],
                                            combo_name=clname,
                                            transaction_cost=0.0005,
                                            rf=0.00,
                                            jz_mode="d",
            ...
            dfpos['jz'] = cum_curve
            # df = bb_func(dfpos,'jz',200,1.5)
            # df = mm_func(dfpos,'jz',60,60)
            # df = ma_func(dfpos,'jz',300,100)
            df = atr_func(dfpos,'jz',120,3.5)
            df['pos'] = df['pos']*df[clname]

            cum_curve, all_metrics = compute_backtest_metrics_with_jz(
                market_df=dfpos[mk_col + ['pos']],
                position_series=dfpos['pos'],
                combo_name='pos',
                transaction_cost=0.0005,
                rf=0.00,
                jz_mode="d",
            ...
            ax1, ax2 = axs
            ax1.plot(df['jz'] )
            ax1.plot(df['close']/df['close'].iloc[0] )
            ax1.plot(cum_curve )
            # ax1.plot(df['up'] )
            # ax1.plot(df['me'] )
            # ax1.plot(df['dn'] )
            ...
            plt.show()
            exit()
        dfpos['mean'] = dfpos[cl_col].mean(axis=1)
        print(dfpos.head())
        plt.plot(dfpos['mean'])
        plt.show()
        exit()
    ...
    if 1:
        ...
        mk_col = [col for col in dfpos.columns if "|" not in col]
        if True:
            dfpos[cl_col]= dfpos[cl_col].map(lambda x: 1 if x>0 else -1)
            dfpos['mean'] = dfpos[cl_col].mean(axis=1)
            dfpos['mean'] = dfpos['mean'].apply(lambda x: np.where(x>0,1,np.where(x<0,-1,0)))


            cum_curve, all_metrics = compute_backtest_metrics_with_jz(
                                            market_df=dfpos[mk_col+['mean']],
                                            position_series= dfpos["mean"],
                                            combo_name='mean',
                                            transaction_cost=0.0005,
                                            rf=0.00,
                                            jz_mode="d",
            ...
            print(all_metrics)
            # plt.plot(cum_curve)
            # plt.plot(dfpos['close']/dfpos['close'].iloc[0])
            # plt.plot(dfpos['mean'])
            # plt.show()

            dfpos['jz'] = cum_curve
            # dfpos['jz_ma'] = talib.SMA(dfpos['jz'],220)
            # dfpos['jz_ma'] = talib.MIN(dfpos['jz_ma'],50)
            # dfpos.loc[dfpos['jz']>dfpos['jz_ma'],'pos'] = dfpos['mean']
            # dfpos.loc[dfpos['jz']<=dfpos['jz_ma'],'pos'] = 0
            # dfpos['pos'] = dfpos['pos'].ffill().fillna(0)
            # dfpos = bb_func(dfpos,'jz',100,3.05)
            # dfpos = mm_func(dfpos,'jz',60,60)
            # dfpos = ma_func(dfpos,'jz',300,100)
            # dfpos = atr_func(dfpos,'jz',320,3.05)
            # dfpos['pos'] = dfpos['pos']*dfpos['mean']
            # dfpos['pos'] = dfpos['pos'].ffill().fillna(0)


            # dfpos['pos'] = np.where(dfpos['pos']>0,1,np.where(dfpos['pos']<0,0,0))
            dfpos['pos'] = np.where(dfpos['mean']>0,1,np.where(dfpos['mean']<0,0,0))
            ...
            ax1, ax2 = axs
            ax1.plot(dfpos['jz'] )
            ax1.plot(dfpos['close']/dfpos['close'].iloc[0] )
            ax1.plot(cum_curve )
            # ax1.plot(df['up'] )
            # ax1.plot(df['me'] )
            # ax1.plot(df['dn'] )
    ...
    if 0:
        ...
        STRATEGY_PARAMS_CONFIG = {symbol_id:
                [{'name': 'EntryFilters','select_count': n1,'combination': 'and','items': pd.read_csv(fr"{dir_data_path}\{symbol_id}_all_filters_summary.csv", usecols=['策略名称'])['策略名称'].tolist()[::2]},
                 {'name': 'EntrySignals','select_count': n2,'combination': 'or','items': pd.read_csv(rf"{dir_data_path}\{symbol_id}_entry_all_validated.csv", usecols=['signal_id'])['signal_id'].tolist()[::2]},
    ...
    if True:
        ...
        code_map = {'GCmain': 'GC2608', 'SImain': 'SI2607', 'HGmain': 'HG2607', 'CLmain': 'CL0608',
                    'ZSmain': 'ZS2611', 'ZLmain': 'ZL2612','ZWmain': 'ZW2609',
                    'ZMmain': 'ZM2607',  'ZCmain': 'ZC2607'}

        all_df = pd.DataFrame()
        for code_id in code_ids:
            pos_path = fr"D:\jason_src\策略多因子系统\15min_全品种优化\backtest_result_data-f-2_s-2_e-2_jzmode-d_new\allresult\{code_id}_实盘参数.csv"
            df = pd.read_csv(pos_path)
            df['coin_name'] = df['coin_name'].apply(lambda x: code_map[code_id])

            print(code_id)
            print( df.tail())
            all_df = pd.concat([all_df, df], axis=0)
        print(all_df.shape)
        all_df.to_csv(fr"D:\jason_src\策略多因子系统\15min_全品种优化\backtest_result_data-f-2_s-2_e-2_jzmode-d_new\allresult\all_222_实盘参数.csv", index=False)
