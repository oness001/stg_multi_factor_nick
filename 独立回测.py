    ...
    def run_single(
        ...
        for idx, single_cl_params0 in enumerate(all_cl_params):
            ...
            try:
                ...
                logger.error(f"策略回测失败：{e} ：{traceback.format_exc()}",exc_info=True)
                continue
...
if __name__ == "__main__":
    ...
    if 1:
        decoded_params = [
            f"aroon_diff_higher0^21|emacd_higher_signal^89^178^89|aroon_diff_higher0^{i}&"
            "adx_plus_crossover_minus^34|tp_low_1^15^0|s_tp_low_1^15^0^2&"
            "trailing_stop^34^3.5|emacd_histogram_start_decrease^55^110^55"   for i in range(50,200,2)]
        # 回测默认配置
        BACKTEST_CONFIG = {
            "transaction_cost": 0.0004,  # 单边交易成本（万三）
        ...
        code_id = "GCmain"
        start, end = "2026-03-01", "2026-06-18"
        ...
        period_cond = (market_df_full["candle_begin_time"] >= start) & (market_df_full["candle_begin_time"] <= end)
