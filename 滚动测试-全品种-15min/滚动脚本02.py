...
if True:
    ...
    SAVE_RAW_FORCE_FILTER_CONFIG = {"total+总收益率%":0,
                                    'total+交易胜率%':45 ,
                                    'total+盈亏比':1.5,
                                    'total+交易次数':5,
                                    'total+最大回撤%':-11}
...
if __name__ == "__main__":
    ...
    OPTIMIZATION_CONFIG = {"population_size": 4000,"n_generations": 400}
    ...
    SAVE_DIR_PATH = rf'D:\jason_src\策略多因子系统\滚动测试-全品种-15min\滚动信号结果'
    ...
    if 0:
        ...
        direction_choose = 1
        filter_path = rf"D:\jason_src\策略多因子系统\滚动测试-全品种-15min\滚动测试文件\{symbol_id}_filters_20250901_20260601_rolling_3_1_全量滚动窗口筛选结果.csv"
        signal_path = rf"D:\jason_src\策略多因子系统\滚动测试-全品种-15min\滚动测试文件\{symbol_id}_signals_20250901_20260601_rolling_3_1_全量滚动窗口筛选结果.csv"
        ...
        for winlabel0 in win_label:
            ...
            STRATEGY_PARAMS_CONFIG_short = {symbol_id: stg_cfg1, **STRATEGY_PARAMS_CONFIG_short}
            ...
            run_optimization_batch(code_ids=[symbol_id],
                                   start=start,
                                   end=end,
        ...
        if True:
            ...
            main(symbol_id, output_dir, METRICS_CONFIG, Force_CONFIG, PCT_N, Force_N, LIMIT_N, LOW_CORR_N, LIMIT_CORR_VAL, BACKTEST_CONFIG)
    ...
    if 1:
        ...
        for symbol_id in code_ids:
            if symbol_id == 'USDCNH':
                ...
                filter_path = rf"D:\jason_src\策略多因子系统\滚动测试-全品种-15min\滚动测试文件\{symbol_id0}_filters_20251001_20260601_rolling_3_1_全量滚动窗口筛选结果.csv"
                signal_path = rf"D:\jason_src\策略多因子系统\滚动测试-全品种-15min\滚动测试文件\{symbol_id0}_signals_20251001_20260601_rolling_3_1_全量滚动窗口筛选结果.csv"
                long_filter_list, short_filter_list, filter_window_labels = get_fliter(filter_path)
                long_signal_list, short_signal_list, signal_window_labels = get_signal(signal_path)
                win_label = set(filter_window_labels) & set(signal_window_labels)
                win_label = sorted(win_label, reverse=True)
                ...
                filter_path = rf"D:\jason_src\策略多因子系统\滚动测试-全品种-15min\滚动测试文件\{symbol_id}_filters_20251001_20260601_rolling_3_1_全量滚动窗口筛选结果.csv"
                signal_path = rf"D:\jason_src\策略多因子系统\滚动测试-全品种-15min\滚动测试文件\{symbol_id}_signals_20251001_20260601_rolling_3_1_全量滚动窗口筛选结果.csv"
                long_filter_list, short_filter_list, filter_window_labels = get_fliter(filter_path)
                long_signal_list, short_signal_list, signal_window_labels = get_signal(signal_path)
                win_label = set(filter_window_labels) & set(signal_window_labels)
                win_label = sorted(win_label, reverse=True)
